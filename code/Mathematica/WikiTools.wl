...
Module[{ fixCycles, makeCycles, cs, f},
	fixCycles=
		StringReplace[#,Shortest["Cycles[{"~~x__~~"}]"]:>StringReplace[x,","->""]]&;
	makeCycles = 
		StringReplace[#,x:Shortest["{"~~DigitCharacter~~__~~"}"]:>cs[x]]&;
	cs[str_] := 
		StringReplace[ str, {"{"->"(", "}"->")",","->" \\ "} ];
	StringJoin[ 
		"\n\nThe fusion rules are invariant under the group generated by the following permutations:\n\n",
		StringReplace[ x:DigitCharacter :> boldDigits[[ ToExpression[x] + 1 ]] ] @
		math[
			"\\{" <> StringTake[ ToString[(fixCycles@*makeCycles@*ToString/@cycles)], {2,-2} ] <> "\\}"
		],
		"\n"
	]
];
